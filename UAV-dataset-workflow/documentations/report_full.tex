...
\hline
mosquitto & 8883 (TLS, external) / 1883 (plain, internal) & MQTT broker. Routes all messages between edge devices and the aggregation service. External port uses TLS with password authentication; internal port is plain for Docker-internal services. \\
\hline
aggregation & 8001 (internal only) & FastAPI service. Subscribes to all \texttt{uav/\#} MQTT topics, maintains the device state registry, and pushes real-time updates to connected browsers via WebSocket. \\
\hline
frontend-builder & — & One-shot build container. Compiles the React/TypeScript frontend into static files and writes them to a shared Docker volume (\texttt{frontend-dist}). Exits after build completes. \\
\hline
control-center & 8080 & FastAPI service. Serves the compiled React frontend, handles JWT authentication, and proxies all \texttt{/api/*} requests to the aggregation service. \\
\hline
signaling & 8090 (all interfaces) & Node.js WebRTC signaling server. Brokers SDP offer/answer and ICE candidate exchange between edge devices and browser clients. Must bind to \texttt{0.0.0.0} so edge devices on the local network can reach it. \\
\hline
ha\_bridge & — (internal) & Silent background service. Forwards UAV detection state to external automation systems without any user-facing interface. \\
\hline
tile-server & 8070 (optional) & Self-hosted OpenStreetMap tile server. Activated with \texttt{--profile tiles}. Serves pre-imported map tiles for offline operation. \\
\hline
\end{tabular}
\end{table}

The internal Docker network (\texttt{uav-net}) allows services to communicate
by container name without exposing ports to the host. The aggregation service
...
\hline
\texttt{uav/status/\{id\}} & Edge $\to$ Main & Online/offline status, active model name, GPS coordinates. Published as a retained message so the control center receives the last known status immediately on connection. QoS 1. \\
\hline
...
\hline
\texttt{uav/ipwebcam/capabilities/\{id\}} & Edge $\to$ Main & Available IP Webcam settings and their valid ranges. Published once on startup. QoS 0. \\
\hline
\texttt{uav/ipwebcam/sensors/\{id\}} & Edge $\to$ Main & Phone sensor data: battery level, temperature, light, motion, pressure. QoS 0. \\
\hline
\texttt{uav/snapshot/\{id\}} & Edge $\to$ Main & Base64-encoded JPEG snapshot from the camera. Published on demand. QoS 0. \\
\hline
\texttt{uav/command/\{id\}} & Main $\to$ Edge & Commands: \texttt{start\_stream}, \texttt{stop\_stream}, \texttt{switch\_model}, \texttt{update\_config}, \texttt{ipwebcam\_control}, \texttt{ipwebcam\_sensors}. QoS 1. \\
\hline
...
broker and delivered immediately to any new subscriber. This ensures that
when the control center restarts or a new browser connects, it immediately
...
\begin{itemize}
  \item \textbf{Stream}: zoom slider (0--100), video resolution dropdown
        ...
        slider.
  \item \textbf{Camera}: torch (flashlight) toggle, front/back camera toggle,
        night vision mode toggle, and timestamp overlay toggle.
