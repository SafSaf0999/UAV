...
The production model, \textbf{BirdDrone-2C}, is a fine-tuned 2-class
Bird/Drone YOLO26s detector trained on curated aerial imagery and subsequently
fine-tuned on supplementary aerial sequences.
...
comparable benchmark. Key findings include:

\begin{itemize}
  \item The 2023 edition reported top methods achieving F1 $\approx$ 0.91 (OBSS, 1st place)
        and 0.88 (multi-scale YOLOv8, IJCNN 2025)~\cite{simd3_2025}.
  \item The challenge uses RGB video sequences with small drone targets against complex
        backgrounds including birds, making bird/drone discrimination the central challenge.
  \item YOLOBirDrone~\cite{yolobirdrone2025} (arXiv:2601.08319) proposes a dedicated dataset
        and enhanced YOLO architecture for bird vs.\ drone classification, reporting improved
        discrimination over standard YOLO baselines.
  \item Their work confirms that the Bird/Drone boundary is the primary challenge in aerial
        detection systems --- the same challenge that BirdDrone-2C is specifically designed
        to address.
\end{itemize}
...
\begin{itemize}
  \item Reis et al.~\cite{reis2024yolov8} demonstrate a two-stage strategy: a generalised YOLOv8
        ...
        BirdDrone-2C base training followed by fine-tuning described in this report.
...
\begin{itemize}
  ...
  \item These findings inform the model selection (YOLO26s over larger variants), the configurable
        ...
        edge hardware in Chapter~6.
\end{itemize}

\section{Distance and Trajectory Estimation}

Distance and trajectory estimation are theoretical extensions that can be layered
on top of a frame-level detector to provide richer situational awareness. The
following describes the mathematical basis for these capabilities.

\textbf{Distance estimation} uses the pinhole camera model to relate the apparent
size of a detected object to its physical distance from the camera:
...
where $W_{\text{real}}$ is the known UAV wingspan (a configurable prior, typically
0.3--1.0 m for consumer drones), $W_{\text{bbox}}$ is the detected bounding box
width in pixels, $W_{\text{frame}}$ is the image width in pixels, and
$\theta_{\text{FOV}}$ is the camera's horizontal field of view. This model assumes
a calibrated camera and a known target size --- both reasonable assumptions for a
fixed installation with a known drone class.

\textbf{Trajectory estimation} computes the mean frame-to-frame centroid
displacement over a rolling window of $N$ frames:

\begin{equation}
\vec{v} = \frac{1}{N-1} \sum_{i=1}^{N-1} \left( \vec{c}_{i+1} - \vec{c}_i \right)
\end{equation}

where $\vec{c}_i$ is the bounding box centroid at frame $i$. The resulting velocity
vector $\vec{v}$ can be used to predict the drone's position in the next frame,
enabling predictive PTZ camera control and early alert generation before the drone
reaches a protected zone.

Both capabilities depend on stable track IDs from the ByteTrack algorithm: distance
estimates are only meaningful for a consistently tracked object, and trajectory
estimation requires a minimum window of $N$ consecutive detections. In practice,
the accuracy of distance estimation is limited by uncertainty in the drone's actual
wingspan and by the assumption of a fronto-parallel viewing angle.
...
of the key prior systems reviewed in this chapter.
...
\hline
Munir~\cite{munir2023kfupm} (2023) & YOLO-RAW (YOLOv5 extension) & Anti-UAV + weather-augmented & 95.2\% mAP@0.5 (clean); 83.3\% under rain & Single-class; no bird discrimination \\
\hline
Coluccia et al.~\cite{coluccia2021sensors} (2021) & Various (challenge) & DvB Grand Challenge & F1 up to 0.91 & No bird/drone class separation reported per-model \\
\hline
Reis et al.~\cite{reis2024yolov8} (2024) & YOLOv8 (multi-size) & 15,064 flying objects & 99.1\% mAP@0.5 (3-class) & No bird class; evaluated on controlled dataset only \\
\hline
Stäcker et al.~\cite{stacker2021} (2021) & RetinaNet-18 (TensorRT) & COCO + custom & 0.361 mAP (Int8, Jetson) & Optimisation study; no domain-specific training \\
\hline
Delleji et al.~\cite{delleji2025thermal} (2025) & YOLOv11 & Custom thermal & High precision/recall on custom set & Thermal only; limited public benchmark evaluation \\
\hline
\textbf{This work (RGB)} & \textbf{YOLO26s (BirdDrone-2C)} & \textbf{BirdDrone-2C + DUT} & \textbf{0.969 mAP@0.5; DR=0.818 on DUT} & \textbf{RGB only; no adverse weather evaluation} \\
\hline
\textbf{This work (Thermal)} & \textbf{YOLO26s (ThermalDrone)} & \textbf{SIDD thermal} & \textbf{0.958 mAP@0.5 (val); F1=0.842 (Anti-UAV410)} & \textbf{1-class; trained on SIDD only} \\
\hline
\end{tabular}
\end{table}
...
benchmarks (DUT Anti-UAV, Anti-UAV410) with explicit bird/drone discrimination.
...
of UAS threats and defenses, cataloguing seven categories of illicit UAS use:

\begin{itemize}
  \item \textbf{Nuisance} --- unauthorised flights over private or restricted areas.
  \item \textbf{Surveillance / Reconnaissance} --- covert intelligence gathering.
  \item \textbf{Airspace interference} --- disruption of manned aviation operations.
  \item \textbf{Kinetic / Kamikaze attacks} --- direct physical impact on targets.
  \item \textbf{Payload / Smuggling} --- delivery of contraband or hazardous materials.
  \item \textbf{Weaponised platforms} --- UAS modified to carry offensive payloads.
  \item \textbf{Electronic attack} --- jamming, spoofing, or cyber intrusion via UAS.
\end{itemize}

Real-world incidents documented in their analysis include a foiled 2011 FBI plot
involving RC aircraft packed with explosives targeting the Pentagon, and a hexicopter
carrying methamphetamines intercepted near the US--Mexico border in 2015.

The paper synthesises 39 unique UAS defense concepts into a five-layer
``Defense in Depth'' model:

\begin{itemize}
  \item \textbf{Prevention} --- policies, regulations, and geofencing to deter unauthorised flights.
  \item \textbf{Deterrence} --- visible countermeasures and signage that discourage incursions.
  \item \textbf{Denial} --- physical or electronic barriers that prevent UAS from entering a zone.
  \item \textbf{Detection} --- sensor systems (visual, radar, acoustic) that identify UAS presence.
  \item \textbf{Interruption / Destruction} --- active countermeasures that neutralise detected threats.
\end{itemize}

Automated visual detection occupies the critical Detection layer that enables all
subsequent active responses. Miasnikov~\cite{miasnikov2005uav} further establishes
that conventional radar systems and surface-to-air missiles are poorly suited to
detecting small, slow, low-altitude propeller-driven UAVs due to their small radar
cross-sections and the clutter-filtering algorithms that discard slow-moving objects.
This gap in traditional air defense directly motivates the need for dedicated
...
\begin{table}[H]
\centering
\caption{Benchmark results from Liu et al.~\cite{liu2024survey} on infrared small-target datasets.}
...
\hline
SIRST  & UIU-Net      & 78.25 & 100.0 & ---  \\
SIRST  & DNA-Net      & ---   & ---   & 2.51 \\
NUDT-SIRST & RepISD-Net & 89.44 & ---  & ---  \\
\hline
...
on all 20 DUT benchmark videos. It is worth noting that even after
fine-tuning, the model continues to struggle in certain scenarios:
\begin{itemize}
  \item \textbf{Urban building backgrounds}: video10 (31 tracking gaps in the base model,
        reduced to 8 after fine-tuning) and video13 (43 gaps reduced to 28) demonstrate
        that building edges and window patterns continue to cause intermittent tracking loss.
  \item \textbf{Hazy / low-contrast sky}: video16 shows a regression after fine-tuning,
        where the model suppresses marginal detections that the base model accepted at low
        confidence.
  \item \textbf{Clean-background regression}: video19 shows a small regression on a
        near-perfect clean-background sequence, suggesting the fine-tuning distribution
        slightly shifts the decision boundary.
\end{itemize}
These residual weaknesses are documented in the Limitations section and addressed
in the short-term future work as targets for weather and background augmentation.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{run_graphs/BirdDrone-2C-finetuned/confusion_matrix_normalized.png}
  \caption{BirdDrone-2C fine-tuned model normalised confusion matrix. The
...
The UAVDetector result (0.273 base, 0.341 fine-tuned) is low in absolute terms
because fixed-wing UAVs have a fundamentally different visual appearance from the
quadcopter-dominated training data: elongated fuselage, swept wings, and a
different aspect ratio at typical detection distances. The base model was not
trained on fixed-wing examples in sufficient quantity to generalise well to this
domain. The +0.069 improvement after fine-tuning suggests the supplementary
training data includes some fixed-wing or elongated aerial objects that partially
bridge this gap, but the low absolute score confirms that fixed-wing detection
remains a weakness that would require dedicated fixed-wing training data to address.
...
This chapter presented the experimental results for the BirdDrone-2C
production model. The key findings are:

\begin{itemize}
  \item Fine-tuning improves mAP@0.5 from 0.926 to 0.969 on the combined validation
...
\section{Why BirdDrone-2C is the Recommended Production Model}

BirdDrone-2C was selected as the production model based on four criteria:
